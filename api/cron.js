...
// ── MAIN EXPORT CHO VERCEL ───────────────────────────────

export default async function handler(req, res) {
    if (!GITHUB_TOKEN) {
        return res.status(500).json({ error: 'Thiếu GITHUB_TOKEN' });
    }
    
    // Bảo vệ endpoint cron bằng authorization header (tuỳ chọn)
    ...
    console.log('\n══════════════════════════════════════════════════');
    console.log('🚀  PHIMFLIX CLOUD AUTO UPDATE (VERCEL CRON)');
    ...
    try {
                ...
                if (slugMap.has(slug)) {
                    ...
                    if (fresh.episodes.length > 0) {
                        ...
                        if (hasChange) {
                            ...
                            stats.skipped++;
                            console.log(`     ⏭️  Không đổi: ${item.name}`);
                        }
                    } else {
                        stats.skipped++;
                    }
        ...
        return res.status(200).json({ success: true, stats, time: elapsed });
    } catch (error) {
